...
import { Row, Col, Card, Table, Modal, Button, InputGroup, Form } from 'react-bootstrap';
...
import '../catTradeType/css/cattradetype.css';
import 'animate.css';
...
function CatTradeType() {
  ...
  const [deleteCatTradeType, setDeleteCatTradeType] = useState(null);
  ...
  let [searchTradeCates, setSearchTradeCates] = useState('');
  const catLimit = 50;

  const fetchCatTradeType = () => {
    setLoading(true);
    axios
      .get(Urls.get.cattradetype + `?page=${activePage}&limit=${limit}&is_delete=no&tradeCategory=${__ID}&q=${search}`)
      .then((res) => {
    ...
    axios.get(Urls.get.tradetype + `?limit=${catLimit}&q=${searchTradeCates}`).then((res) => {
      setTradeTypeData(res.data?.data?.docs || []);
    });
  };

  const deleteItem = () => {
    axios.delete(Urls.delete.deleteCatTradeType + deleteCatTradeType).then(() => {
  ...
  useEffect(() => {
    fetchCatTradeType();
  }, [activePage, limit, search, __ID, searchTradeCates]);
  ...
  const handleTradeTypeChange = (selectedOption) => {
    setSelectedTradeType(selectedOption);
    setID(selectedOption ? selectedOption.value : ''); // Update tradeCategory ID
  };
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=tradeTypeCat`, payload).then((res) => {
  ...
  //   formData.append('TradeTypeCat', selectedFile); // 'Country' is the key expected by the API

  //   try {
  //     setLoadingUpload(true);

  //     const response = await axios.post(Urls.post.tradeTypeCategoryImport, formData, {
  //       headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="CatTradeType delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainCategory animate__animated animate__fadeInRight">
        <Col>
              ...
              <Row>
                <div className="col-4 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>Trade Type Category</b>
                  </Card.Title>
                </div>
                <div className="col-6">
                  <Row>
                    <Form.Group className="mb-3" as={Col} controlId="exampleForm.ControlSelect1">
                      <Select
                        ...
                        options={[
                          { label: 'All Trade Type', value: '' }, // Represents selecting all
                ...
                <div className="col-2 d-flex">
                  <div>
                    <Link to={'/masters/AddCatTradeType'}>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            <img src={imgUrl + item?.tradeCategory?.icon} style={{ width: '30px', height: '30px' }} alt="" />
                          </td>
                          ...
                          <td className="text-center">
                            <Link to={location?.state?.activePage || 1}>
                              <Button
                                ...
                                onClick={() => {
                                  setShow(true);
                                  setDeleteCatTradeType(item._id);
                                }}
                              >
                            ...
                            <Link to={'/masters/EditCatTradeType/' + item._id} state={{ activePage, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the CatTradeType ?</Modal.Body>
          <Modal.Footer>
