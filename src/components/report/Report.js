...
export default function Reports() {
  ...
  function getReports() {
    setLoading(true);
    axios
      .get(Urls.get.reports + `?page=${page}&limit=${limit}&q=${search}&is_delete=no`)
      .then((res) => {
  ...
  const deleteItem = () => {
    axios.delete(Urls.delete.deleteReason + deleteRecord).then(() => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=report`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    ...
    formData.append('Report', selectedFile); // 'Country' is the key expected by the API

    try {
      setLoadingUpload(true);

      const response = await axios.post(Urls.post.reportImport, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });

      console.log(response.data.message || 'File imported successfully!');
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="Reason delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainReport animate__animated animate__fadeInRight">
        <Col>
              ...
              <Row>
                <div className="col-7 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>Report Table</b>
                  </Card.Title>
                </div>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/report/editReport' + '/' + item._id} state={{ page, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the reason ?</Modal.Body>
          <Modal.Footer>
