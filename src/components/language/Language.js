...
import '../language/css/language.css';
...
function Language() {
  ...
  const fetchData = () => {
    setLoading(true);
    axios
      .get(Urls.get.language + `?page=${activePage}&limit=${limit}&q=${search}`)
      .then((res) => {
  ...
  const deleteItem = () => {
    axios.delete(Urls.delete.deleteLanguage + deleteLanguage).then((res) => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=language`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    ...
    formData.append('Language', selectedFile); // 'Country' is the key expected by the API

    try {
      setLoadingUpload(true);

      const response = await axios.post(Urls.post.languageImport, formData, {
        headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="Language delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainLangauage animate__animated animate__fadeInRight">
        <Col>
              ...
              <Row>
                <div className="col-7 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>Language Table</b>
                  </Card.Title>
                </div>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/masters/EditLanguage' + '/' + item._id} state={{ activePage, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the language ?</Modal.Body>
          <Modal.Footer>
