...
import '../country/css/country.css';
...
function Country() {
  ...
  const fetchCountry = () => {
    setLoadingData(true);
    axios
      .get(Urls.get.countries + `?page=${activePage}&limit=${limit}&q=${search}&is_delete=no`)
      .then((res) => {
        setTotalPages(res?.data?.getAllCountries?.totalPages);
        setActivePage(res?.data?.getAllCountries?.page);
        setData(res?.data?.getAllCountries?.docs);
        setTotaldocs(res?.data?.getAllCountries?.totalDocs);
      })
      .catch((err) => {})
  ...
  const deleteItem = () => {
    axios.delete(Urls.delete.deleteCountry + deleteCountry).then((res) => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=country`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    if (!selectedFile) {
      // setUploadStatus('Please select a CSV file to import.');
      setEmptyMsg(true);
      setTimeout(() => {
        setEmptyMsg(false);
      }, 2000);
    ...
    formData.append('Country', selectedFile); // 'Country' is the key expected by the API

    try {
      setLoading(true);
      const response = await axios.post(Urls.post.coutriesImports, formData, {
        headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="Country delete successfully ❌" />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainInterest animate__animated animate__fadeInRight">
        <Col>
            ...
            >
              <Row>
                <div className="col-4 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>Country Table</b>
                  </Card.Title>
                </div>
              ...
              <Row>
                ...
                <div className="col-2 pt-2">
                  <button className="btn  btn-sm btn-danger mt-4" onClick={handleFileUpload} disabled={loading}>
                    {loading ? 'Importing...' : 'Import'}
                  </button>
                </div>

                {/* {uploadStatus && <p style={{ color: uploadStatus.includes('success') ? 'green' : 'red' }}>{uploadStatus}</p>} */}
              </Row>
            </Card.Header>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/masters/EditCountry/' + item?._id} state={{ activePage, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the country ?</Modal.Body>
          <Modal.Footer>
