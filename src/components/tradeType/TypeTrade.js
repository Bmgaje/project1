...
import '../tradeType/css/tradetype.css';
import 'animate.css';
...
function TypeTrade() {
  ...
  const [deleteTradeType, setDeleteTradeType] = useState(null);
  ...
  const fetchTradeType = () => {
    setLoading(true);
    axios
      .get(Urls.get.tradetype + `?page=${activePage}&limit=${limit}&q=${search}&is_delete=no`)
      .then((res) => {
  ...
  const deleteItem = () => {
    axios.delete(Urls.delete.deleteTradeType + deleteTradeType).then((res) => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=tradeCategory`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    ...
    formData.append('tradeCategory', selectedFile);

    try {
      setLoadingUpload(true);

      const response = await axios.post(Urls.post.tradeTypeImport, formData, {
        headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="TradeType delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainTradeType animate__animated animate__fadeInRight">
        <Col>
              ...
              <Row>
                <div className="col-7 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}> Trade Type Table</b>
                  </Card.Title>
                </div>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/masters/EditTradeType/' + item._id} state={{ activePage, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the TradeType</Modal.Body>
          <Modal.Footer>
