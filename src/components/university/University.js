...
import '../university/css/university.css';
...
function University() {
  ...
  let [searchUniversity, setSearchUniversity] = useState('');
  let universityLimit = 50;

  const fetchData = () => {
    setLoading(true);
    axios
      .get(Urls.get.university + `?page=${activePage}&limit=${limit}&q=${search}&country_id=${countryId}&is_delete=no`)
      .then((res) => {
  ...
  useEffect(() => {
    function allCountry() {
      axios.get(Urls.get.countries + `?limit=${universityLimit}&q=${searchUniversity}`).then((res) => {
        setCountryData(res?.data?.getAllCountries?.docs || []);
      });
    }
    allCountry();
  }, [searchUniversity, universityLimit]);
  ...
  const handleSearchInput = (inputS) => {
    setSearchUniversity(inputS);
    setActivePage(1);
  };

  const deleteItem = () => {
    axios.delete(Urls.delete.deleteUniversity + deleteUniversity).then((res) => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=university`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    ...
    formData.append('University', selectedFile); // 'Country' is the key expected by the API

    try {
      setLoadingUpload(true);

      const response = await axios.post(Urls.post.universityImport, formData, {
        headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="University delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainUniversity animate__animated animate__fadeInRight">
        <Col>
              ...
              <Row>
                <div className="col-4 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>University Table</b>
                  </Card.Title>
                </div>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/masters/EditUniversity/' + item._id} state={{ activePage, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the university ?</Modal.Body>
          <Modal.Footer>
