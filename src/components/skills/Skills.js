...
function Skills() {
  ...
  const [deleteSkills, setDeleteSkills] = useState(null);
  ...
  let [__ID, setID] = useState('');
  let skillLimit = 50;

  const fetchData = () => {
    setLoading(true);
    axios
      .get(Urls.get.skills + `?page=${activePage}&limit=${limit}&q=${search}&skillCategory_id=${__ID}`)
      .then((res) => {
        setTotalPages(res?.data?.getAllSkills?.totalPages);
        setActivePage(res?.data?.getAllSkills?.page);
        setData(res?.data?.getAllSkills.docs);
        setTotaldocs(res?.data?.getAllSkills?.totalDocs);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const deleteItem = () => {
    axios.delete(Urls.delete.deleteSkills + deleteSkills).then((res) => {
  ...
  useEffect(() => {
    fetchData();
  }, [activePage, limit, search, __ID]);

  // Handle Trade Type Selection
  const handleSkillCateChange = (selectedOption) => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=skills`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    ...
    formData.append('Skill', selectedFile); // 'Country' is the key expected by the API

    try {
      setLoadingUpload(true);

      const response = await axios.post(Urls.post.skillsImport, formData, {
        headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="Skill delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainSkills animate__animated animate__fadeInRight">
        <Col>
              ...
              <Row>
                <div className="col-4 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>Skills Table</b>
                  </Card.Title>
                </div>
                ...
                <div className="col-3 d-flex">
                  <Form.Group className="mb-3" as={Col} controlId="exampleForm.ControlSelect1">
                    <Select
                      value={selectedSkill}
                      onChange={handleSkillCateChange}
                      options={[
                        { label: 'All Skill Category', value: '' }, // Represents selecting all
                        ...skillCateData.map((item) => ({
                          label: item.title,
                          value: item._id
                        }))
                      ]}
                  ...
                  <Form.Group className="mb-3" as={Col} controlId="exampleForm.ControlSelect1">
                    <InputGroup>
                      <Form.Control
                        ...
                        size="sm"
                        onChange={handleSearchSkillCate}
                        type="text"
                    ...
                    return (
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td>{item?.skill_category?.title ? item?.skill_category?.title : 'NA'}</td>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/masters/EditSkills' + '/' + item._id} state={{ activePage, limit }} className="text-white">
