...
import '../interest/css/interest.css';
...
function Interest() {
  ...
  const fetchData = () => {
    setLoading(true);
    axios
      .get(Urls.get.interest + `?page=${activePage}&limit=${limit}&q=${search}&is_delete=no`)
      .then((res) => {
        setTotalPages(res?.data?.getAllInterest?.totalPages);
        setActivePage(res?.data?.getAllInterest?.page);
        setData(res.data?.getAllInterest.docs);
        setTotaldocs(res?.data?.getAllInterest?.totalDocs);
      })
      .finally(() => setLoading(false));
  };

  const deleteItem = () => {
    axios.delete(Urls.delete.deleteInterest + deleteInterest).then((res) => {
  ...
  const handleStatus = (e) => {
    ...
    axios.post(Urls.post.addStatus + '/' + __id + `?type=interest`, payload).then((res) => {
  ...
  const handleFileUpload = async () => {
    ...
    formData.append('Interest', selectedFile); // 'Country' is the key expected by the API

    try {
      setLoadingUpload(true);

      const response = await axios.post(Urls.post.interestImport, formData, {
        headers: {
    ...
    <div>
      ...
      <div className="row position-relative">
        <div className="col-3 offset-9 position-fixed z-3" style={{ top: '60px', right: '20px' }}>
          {deleteNotification ? (
            <b>
              <Notification message="Interest delete successfully." />
            </b>
          ) : null}
        </div>
      </div>
      ...
      <Row className="mainInterest animate__animated animate__fadeInRight">
              ...
              <Row>
                <div className="col-7 pt-1">
                  <Card.Title as="h4">
                    ...
                    />
                    <b style={{ color: '#10446c' }}>Interest Table</b>
                  </Card.Title>
                </div>
                      ...
                      <tbody key={index} className="tableBody">
                        <tr>
                          ...
                          <td className="text-center">
                            ...
                            <Link to={'/masters/EditInterest' + '/' + item._id} state={{ activePage, limit }} className="text-white">
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>Are you sure want to delete the interest ?</Modal.Body>
          <Modal.Footer>
